...
const EditStoreModal = ({ store, onClose, onSave }) => {
    ...
    useEffect(() => {
        if (store) {
            setFormData({
                ...
                open_dt: store.open_dt || '',
                close_dt: store.close_dt || '',
                enable: store.enable === 1,
            });
            setPaymentInfo({
                bizNumber: store.bizNumber || '',
                vanType: store.vanType || '',
                PG_MERCHANT_ID: store.PG_MERCHANT_ID || '',
                PG_PRIVATE_KEY: store.PG_PRIVATE_KEY || '',
                PG_PAYMENT_KEY: store.PG_PAYMENT_KEY || '',
            });
        }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        onSave(store.id, { ...formData, ...paymentInfo });
    };
    ...
    return (
        <div className="modal" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <div className="modal-content" style={{ width: '1500px !important', minHeight: '500px', display: 'flex', flexDirection: 'column', gap: '0' }}>
                {/* 상단: 가로 2분할 */}
                <div style={{ display: 'flex', flexDirection: 'row', gap: '32px', flex: 1 }}>
                    {/* 왼쪽: 매장 정보 */}
                    <div style={{ flex: 1, paddingRight: '24px', borderRight: '1px solid #eee' }}>
                        <h2>매장 수정</h2>
                        <div className="form-group">
                            <label htmlFor="editStoreName">매장명</label>
                            <input type="text" id="editStoreName" name="name" value={formData.name} onChange={handleChange} required />
                        </div>
                        <div className="form-group">
                            <label htmlFor="editStorePhone">연락처</label>
                            <input type="text" id="editStorePhone" name="phone" value={formData.phone} onChange={handleChange} required />
                        </div>
                        <div className="form-group">
                            <label htmlFor="editStoreAddress">주소</label>
                            <input type="text" id="editStoreAddress" name="address" value={formData.address} onChange={handleChange} required />
                        </div>
                        <div className="form-group">
                            <label htmlFor="editStoreGroup">그룹</label>
                            <select id="editStoreGroup" name="store_group_id" value={formData.store_group_id} onChange={handleChange}>
                        ...
                        <div className="form-group">
                            <label htmlFor="editStoreOpenDt">개점시간</label>
                            <input type="time" id="editStoreOpenDt" name="open_dt" value={formData.open_dt} onChange={handleChange} required />
                        </div>
                        <div className="form-group">
                            <label htmlFor="editStoreCloseDt">폐점시간</label>
                            <input type="time" id="editStoreCloseDt" name="close_dt" value={formData.close_dt} onChange={handleChange} required />
                        </div>
                        <div className="form-group form-group-inline">
                            <label>활성화</label>
                            <label className="switch">
                                <input type="checkbox" id="editStoreEnable" name="enable" checked={formData.enable} onChange={handleChange} />
